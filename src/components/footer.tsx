        ...
        <div className="mt-6 flex flex-col gap-3 border-t border-white/10 pt-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <nav aria-label="Redes sociais" className="flex items-center gap-2">
            {socialLinks.map(({ label, href, Icon }) => (
              <Link key={label} href={href || "#"} target="_blank" rel="noopener noreferrer" aria-label={`Abrir ${label}`} className="flex size-8 items-center justify-center rounded-full text-amparo-200 outline-amparo-400 transition hover:bg-white/10 hover:text-amparo-400 focus-visible:outline-2 focus-visible:outline-offset-2"><Icon className="size-4" /></Link>
            ))}
          </nav>
        </div>
      </div>
